...
old_path = os.getcwd()
abs_file_path = sys.argv[0]
abs_path = os.path.dirname(abs_file_path)
abs_name = os.path.basename(abs_file_path)
abs_end = os.path.splitext(abs_file_path)[-1]
abs_pid = os.getpid()
...
# 导入运行库
try:
    import datetime, json, traceback, shutil, re, time, hashlib, threading, ctypes, stat, bs4, lxml, urllib.parse, requests, send2trash, winshell, platform, webbrowser, win32api, win32con, win32com.client, random
except:
    for i in lib_list:
        pipInstall(i)
    os.popen("main.pyw")
