...
# 搜索模组多
def search(data=None, versions=None, limit=20, page=1):
    if not data:
        data = None
    if not versions:
        versions = None
    ...
    if limit < 1:
        ...
        limit = 100
    page = limit * (page - 1)
    facets = facetsEdit("mod", versions)
    response = requests.get(joinUrl("https://api.modrinth.com/v2/search", {"query": data, "facets": facets, "limit": limit, "index": "relevance", "offset": page}), headers=header, stream=True).text
